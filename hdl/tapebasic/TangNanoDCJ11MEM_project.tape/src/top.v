...
// for PCB rev.2.x and rev.3.x (and patched rev.1.1)
//
// version 20250922.papertape
// This HDL is for using PC-11(Paper-Tape Reader/Punch) emulator
// instead of RF11 and RK11
//
...
//
// 2025/09/15: - ws2812 module updated for LED array
// 2025/09/17: - bugfix for using without SD memory
// 2025/09/21: - sw2_toggle and sw2_count installed for debug
...
//---------------------------------------------------------------------------
  ...
  // This application uses SD memory as Paper Tape emulator
  // so HD emulator is not used.
  // The compiler will sweep out sdhd module.
  parameter	 SDHD_CLK_FRQ = SYS_CLK_FRQ;
//  wire		 sdhd_clk     = sys_clk;
  wire		 sdhd_clk     = 1'b0;

  // clock for sdtape
  parameter	 SDTAPE_CLK_FRQ = SYS_CLK_FRQ;
  wire		 sdtape_clk     = sys_clk;
...
//---------------------------------------------------------------------------
  reg		 RESET_n; // Reset for memory system and UART
...
//---------------------------------------------------------------------------
// make toggle switch sw2_toggle and 3-bit counter sw2_count from sw2_n
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
  assign DAL = BUFCTL_n ? 16'bzzzz_zzzz_zzzz_zzzz :
       ...
       (address == ADRS_PRS)  ? {PRS_ERROR, 3'b0, PRS_BUSY,
				 3'b0, PRS_DONE, PRS_IE, 6'b0}:
       (address == ADRS_PRB)  ? {8'b0, PRB_DATA}:
       (address == ADRS_PPS)  ? {PPS_ERROR, 7'b0, PPS_READY,
				 PPS_IE, 6'b0}:
...
// mem_hi and mem_lo have 32KW capacity for fail safe
  ...
  wire	      we_hi = write_memory_hi;
  wire	      we_lo = write_memory_lo;
...
//wire ram_area = (wa[14:12] != 3'b111); // 160000-177777 is ROM (RAM=28KW)
  ...
  always @(posedge sys_clk)
    if( ram_area ) begin 
	 ...
	 mem_lo[wa] <= d_cpu_to_ram[7:0];
       if(we_hi)
	 mem_hi[wa] <= d_cpu_to_ram[15:8];
    end
...
//---------------------------------------------------------------------------
  wire		 PRS_ERROR; // bit15
  wire		 PRS_BUSY;  // bit11
  wire		 PRS_DONE;  // bit7
  reg		 PRS_IE;    // bit6
  wire [7:0]	 PRB_DATA;  // bit7..0

  wire		 PPS_ERROR; // bit15
  wire		 PPS_READY; // bit7
  reg		 PPS_IE;    // bit6
  ...
  assign PRS_ERROR = (sd_error != 0);
  assign PPS_ERROR = (sd_error != 0);

  always @(negedge SCTL_n or negedge INIT_n) // write to PRS
    if( ~INIT_n)
      {tape_read, PRS_IE} <= 0;
    else if((address == ADRS_PRS) & bus_write) begin
       tape_read <= DAL[0];  // set reader enable and update PRB_DATA
...
//    if( address[17:1] == (ADRS_PRB>>1) )
//      tape_clear_done <= 1'b1;
//    else if( ~PRS_DONE )
//      tape_clear_done <= 1'b0;
  
  always @(posedge sys_clk) // read tape buffer
    if( (address[15:1] == (ADRS_PRB>>1)) & bus_read )
      tape_clear_done <= 1'b1;
    else if( ~PRS_DONE )
      tape_clear_done <= 1'b0;

  always @(negedge SCTL_n or negedge INIT_n) // write to PPS
    if( ~INIT_n)
      PPS_IE <= 0;
...
//---------------------------------------------------------------------------
  ...
  wire [3:0]  sd_error;
  wire [4:0]  sd_state;
  wire [30:0] sd_tape_address;
   
  sdtape #(
	      .SYS_FRQ(SDTAPE_CLK_FRQ),
	      .MEM_FRQ(400_000)
    ) sdtape_inst
  (
   .i_clk              (sdtape_clk),
   .i_reset_n          (RESET_n),
   .i_sd_det_n         (1'b0),      // TangNano20K doesn't have sd_det
   .i_sd_miso          (sd_miso),
   .o_sd_mosi          (sd_mosi),
   .o_sd_cs_n          (sd_cs_n),
   .o_sd_clk           (sd_clk),
   .o_tape_read_busy   (PRS_BUSY),
   .o_tape_read_done   (PRS_DONE),
   .o_tape_punch_ready (PPS_READY),
   .i_tape_read        (tape_read),
   .i_tape_punch       (tape_punch & 1'b0), // disable punch
   .i_tape_clear_done  (tape_clear_done),
   .i_tape_punch_data  (PPB_DATA),
   .o_tape_read_data   (PRB_DATA),
   .i_tape_flush       (1'b0),              // disable punch
   .o_sd_state         (sd_state),
   .o_sd_error         (sd_error),
   .o_sd_tape_address  (sd_tape_address)
...
// monitor sys_clk and ALE_n
  ...
  always @(posedge sys_clk) begin
     ...
     case ( sw2_count )
       1: begin
	  led_array_r[31:16]  <= sd_state;
	  led_array_r[15:0]   <= sd_error;
       end
       2: begin
	  led_array_r[31:16]  <= ADRS_DBG0;
	  led_array_r[15:0]   <= REG_DBG0;
       end
       3: begin
	  led_array_r[31:16]  <= ADRS_DBG1;
	  led_array_r[15:0]   <= REG_DBG1;
       end
       4: begin
	  led_array_r[31:16]  <= ADRS_DBG2;
