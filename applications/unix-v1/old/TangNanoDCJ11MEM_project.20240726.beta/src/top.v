...
// 2024/07/26: - Stabilized disk ready logic for IRQ_RFRK (dirty workaround)
...
//`define USE_ABORT    // access to 160000-167777 or 177700 causes bus error
//`define USE_RAM28KW  // make 160000-177777 read only (except for I/O)
...
`endif
  ...
  always @(posedge sys_clk)
    if(bus_write | bus_read)
      if((address == 16'o177700) |   // bus error check in power up sequence
	 (address[15:12] == 4'b1110) // 'o16xxxx
	 )
	bus_error <= 1'b1;
      else 
	bus_error <= 0;
...
`else
  wire	ram_area = 1'b1; // no ROM area (RAM=32K)
`endif
  always @(posedge sys_clk)
    if( ram_area ) begin 
  ...
  always @(posedge sys_clk)
    if ( dbg_clear )
	 ...
	 dbg_print<= 1'b1;
      ...
      else if ((address == 16'o000204) & bus_read ) begin
