...
## とりあえず動かすための手順
- クロック用の水晶を4MHzにする
- IRQ、EVENT用に下記ジャンパ配線をする。HALTはデバッグ用なので任意。HALTはスイッチと競合するので1kΩの抵抗を付けます。(20240712版以前はIRQ1とIRQ2を使用していましたが，IRQ0とIRQ1が正しいです．)
```
DCJ11       TangNano5V
...
### 命令ログ
  - 177000〜177036番地に，HALTする直前にフェッチしていた命令のアドレスを16個記録しています．原因不明のHALTが起きたときの解析用です．(プリフェッチしているので，実行ではなくフェッチです．時系列は177000→177036の順．)
